...
##
##
## redefinition of read.xls (cannot remember why)
##
##


# findPerl <- gdata:::findPerl
# read.xls <- 
#   function (xls, sheet = 1, verbose = FALSE, pattern, na.strings = c("NA", 
#                                                                      "#DIV/0!"), ..., 
...
#     con <- xls2sep(xls, sheet, verbose = verbose, 
#                    ..., method = method, 
#                    perl = perl)
...
#             "... ")
#       idx <- grep(pattern, readLines(tfn, encoding = encoding))
#       if (length(idx) == 0) {
  ...
  shiny.input = NULL)
{
  ...
  xls.file  <- paste(data.file, ".xls",   sep = "")
  ...
  if(prior){
    ...
    sheet.names <- sheetNames(xls.file)
    sheet.count <- sheetCount(xls.file)
    
    for(i in 1:sheet.count) {
      meta.sheets[[i]] <- read.xls(xls.file, sheet = i,
                                   fileEncoding = "latin1",
                                   method = "tab",
                                   stringsAsFactors = FALSE)
    }
    names(meta.sheets) <- sheetNames(xls.file)[1:sheet.count]
  ...
  if(prior){ 
    if(format == "wide"){
      ...
      row.names(metadata.xls) <- metadata.xls[, idvar]
  ...
  WriteXLS(names(meta.sheets), ExcelFileName = xls.file,
           SheetNames = names(meta.sheets), 
           BoldHeaderRow = TRUE, 
           Encoding = "UTF-8",
           envir = list2env(meta.sheets),
...
growth.function <- function(data, cut = 0.025,
  ...
  tc.mat <- matrix(NaN, ncol = 6, nrow = length(unique(temp$type)))
